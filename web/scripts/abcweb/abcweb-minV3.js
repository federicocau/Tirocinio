...
(function () {
    function W() {
        opt = {jump: 0, no_menu: 0, repufld: 0, noplyr: 0, nocsr: 0, media_height: "", btns: 1, ipadr: "", mstr: 0, autscl: 0, ctrmed: 0, ctrnot: 0, lncsr: 0, opacity: .2, synbox: 0, speed: 1, top_margin: 0, yubvid: "", nomed: 0, delay: 0, repskip: 0, spdctl: 0, lopctl: 0, metro: 0};
    ...
    function q(b, c, d, f, e, l, g) {
        ...
        // posizione asse y del numero indicante i movimenti
        ...
        "undefined" == typeof lpRec && (lpRec = {loopBtn: 1, loopStart: 0, loopEnd: f[f.length - 1]}, $("#lopctl").prop("checked", !1), opt.lopctl = 0);
        this.hmargin = 100;
        this.vmargin = 50;
    ...
    function Y() {
        var b = $("#abclbl"), c = b.html(), d = $("#impbox").prop("checked");
        b.toggleClass("abcimp", d);
        b.html(d ? c.replace("score file", "<b>import</b>") : c.replace("<b>import</b>", "score file"));
        d && !opt.btns && $("#btns").click();
    }

    function ta(b, c) {
        if (0 > b.indexOf("//# This page"))
            alert("not a preload file");
        else {
            b = c.replace(/\n/g, "");
            var d = b.match(/offset_js = (.*);/);
            1 < d.length && (n = offset_js = parseFloat(d[1]));
            d = b.match(/times_arr = (.*);abc_arr/);
            1 < d.length && (times = Z(JSON.parse(d[1])));
            k && (k.times = times, k.times.unshift(0));
    ...
    function aa(b) {
        var c = b.slice(0, 4E3);
        $("#impbox").prop("checked") ? ta(c, b) : 0 <= c.indexOf("//# This page") ? (W(), eval(b), y()) : 0 <= c.indexOf("X:") ? ba(b) : -1 == c.indexOf("<?xml ") ? alert("not an xml file nor an abc file") : (b = $.parseXML(b), b = vertaal(b, {p: "f", t: 1, u: opt.repufld, v: 3}), b[1] && $("#err").append(b[1] + "\n"), ba(b[0]))
    }
    function ua(b) {
        $("#err").text("");
    ...
    function ca(b, c) {
        ...
        var f = "dd" == b ? c[0] : $("#fknp").prop("files")[0];
        f && (z = f.name.split(".")[0], d.readAsText(f))
    }
    function va(b) {
        b.stopPropagation();
        b.preventDefault();
        ...
        /video|audio/.test(b[0].type) ? R("dd", b) : ca("dd", b)
    }

    function R(b, c) {
        var d, f;
        "dbx" == b ? (d = c[0], f = d.link) : (d = "dd" == b ? c[0] : $("#mknp").prop("files")[0], f = window.URL.createObjectURL(d));
        F(d.name, f)
    }

    function wa() {
        $("#yubid")[0].checkValidity() ? (opt.yubvid = $("#yubid").val(), F("", "")) : alert("The youtube video id should be 11 characters long,\neach from 'A' to 'Z', 'a' to 'z', '0' to '9', '-' or '_'")
    }
    ...
    function X(b, c, d) {
        for (w = [];
                b <= c + .001;
                b += d)
            b = Math.round(100 * b) / 100, w.push(b)
    }
    function da(b) {
        function c(b) {
        ...
        b && (ea = b);
        "undefined" == typeof YT ? (c(!0), $("#yubuse").prop("checked", !1), $.getScript("https://www.youtube.com/iframe_api")) : (c(!1), ea())
    }

    function F(b, c) {
        c = c.replace("www.dropbox", "dl.dropboxusercontent").split("?")[0];
        Q = 0 == c.indexOf("http") ? c : b;
        var d;
        b = b.split("?")[0];
        $("#vid, #aud").attr("src", "");
        G && G.stopVideo();
        ...
        if (b) {
            ...
            d.on("loadedmetadata", t);
            X(.5, 2, .05);
            A(0);
            t()
        } else
            p = 1, $("#vid, #aud").css("display", "none"), $("#vidyub").css("display", "inline-block"), $("#buttons").toggleClass("video", !0), da(function () {
                h = G;
                w = h.getAvailablePlaybackRates();
                A(0);
                t();
                h.cueVideoById({videoId: opt.yubvid, startSeconds: n})
            })
    }
    function fa() {
    ...
    function S() {
        var b = parseInt($("body").css("width")) / 2, c = parseInt($("#vid").css("width")) / 2, d = parseInt($("#vidyub").css("width")) / 2, f = parseInt($("#aud").css("width")) / 2;
        $("#vid").css("margin-left", opt.ctrmed ? (b - c).toFixed() + "px" : "0px");
        $("#vidyub").css("margin-left", opt.ctrmed ? (b - d).toFixed() + "px" : "0px");
        $("#aud").css("margin-left", opt.ctrmed ? (b - f).toFixed() + "px" : "0px");
        $("#meddiv").css("text-align", opt.ctrmed ? "left" : "center")
    }
    function t() {
        h && $(h).toggle(!opt.noplyr);
        $("#buttons").toggleClass("noheight", !!opt.noplyr);
        var b = $("#btns").prop("checked"), c = parseFloat($("#buttons").css("height")), d = parseFloat($("body").css("height")), b = b ? parseFloat($("#err").css("height")) : 0, d = 100 - (100 * (c + b) / d).toFixed();


        /* $("#notation").css("height",40+"%"): modificare altezza spartito*/
    ...
    function ba(b)
    {
        ...
        function d(b) {
            ...
            c += "</defs>\n%%endsvg";
            var d = {diamond: 1, triangle: 1, rectangle: 1};
            c = [c];
        ...
        var z = function (b) {
            ...
            c = b[0] + "X:" + c[0];
            b = c.split(/\r\n|[\n\r\u0085\u2028\u2029]/);
            for (c = 0; c < Math.min(100, b.length); ++c)
                (d = b[c].match(/%%scale\s*([\d.]+)/)) && 1 == d[1] && (b[c] = "%%scale 0.99");
            return b
        }
        (b);
        b = z.join("\n");
        0 <= b.indexOf("percmap") && (b = d(b));
        l = new Abc({img_out: function (b) {
                -1 != b.indexOf("<svg") && (b = b.replace(/width="(\d*)px"\s*height="(\d*)px"/,
                ...
                f += b
            }, errmsg: function (b, c, d) {
                e += b + "\n"
            }, read_file: function (b) {
                return""
            }, anno_start: function (b, c, d, e, f, h, k) {
                "note" != b && "rest" != b || r.push(l.sx(e));
                "bar" == b && (e = l.sx(e).toFixed(2), f = l.sy(f).toFixed(2), g[e] = 1, s[f] = 1, p = l.sx(0).toFixed(2), g[p] = 1)
                ...
                for (; b; b =
                    ...
                    if (0 == b.v) {
                        for (c = b.extra;
                                c;
                                c = c.next)
                            14 == c.type && c.tempo_notes && (e = c.tempo_notes.reduce(function (b, c) {
                                return b + c
                            }), e = e * c.tempo / 60);
                        switch (b.type) {
                            case 8:
        ...
        for (b = 0; b < m.length; ++b) {
            ...
            B[b] = y.xs;
            A[b] = y.ys[0];
            C[b] = y.ys[y.ys.length - 1]
        }
        (function () {
            for (var b = 0, c = 1, d = m[b].xs.length, e = 0, f = 0, g = 1, l = 0, h = 1, k = 0; ; ) {
                var s = ga[e - 1];
                if (s = s ? s.match(/[,\d]*(\d)/) : null)
                    s = parseInt(s[1]), s != k && (k = s);
                if (!k || k >= h)
                    f += w[e], x.push(f), v.push(ha[e]), t[b] || (t[b] = []), t[b][c] || (t[b][c] = []), t[b][c][h] = I.length, I.push([b, c, h]);
                "|" != D[e] && (k = 0);
                if ((s = /^:/.test(D[e])) && 1 == h &&
                        !opt.repskip)
                    h = 2, e = l, c = I[g][1], b = I[g][0], d = m[b].xs.length;
                else if (s && (h = 1), /:$/.test(D[e]) && (g = I.length, l = e + 1, h = 1), e += 1, c += 1, c >= d) {
                    c = 1;
                    b += 1;
        ...
        if ("undefined" != typeof endtime_js)
            var F = endtime_js - n, G = x[x.length - 1], x = x.map(function (b) {
            return b * F / G
        });
        L = z;
    ...
    function H() {
        if (h) {
            var b = (p ? h.getCurrentTime() : h.currentTime) - n, c = b;
            0 > b && (b = 0);
    ...
    function xa(b) {
        b.preventDefault();
        b.stopPropagation();
        var c = u.get().indexOf(this);
        b = b.clientX;
        b -= $(this).position().left;
        k.x2time(b, c)
    }
    function ja() {
        $("#sync_out").css("display",
    ...
    function za(b, c) {
        function d() {
            e <= f && (v = setTimeout(d, l), k.tiktak.text(e), e += 1)
        }
        var f, e, l;
        clearInterval(v);
        v = 0;
    ...
    function K(b, c) {
        if (h) {
            ...
            if (g && f || !g && !f) {
                ...
                p ? h.playVideo() : h.play();
                opt.metro && k && (k.time_ix = 0)
            } else
                p ? 5 != l && h.pauseVideo() : h.pause(), opt.metro && na();
            f = !opt.lncsr && !f;
            k && k.time2x(e - n, f)
        }
    }
    ...
    function oa() {
        ...
        opt.synbox = 1;
        q = lpRec.loopBtn ? "lpRec = " + JSON.stringify(lpRec) + ";\n" : "";

        for (c = k.times.map(function (b) {
            return b.toFixed(2)
        }); c.length; )
            g = c[9], d.push(l + c.splice(0, 10).join(",") + "]"), l = "[" + g + ",";
        c = "times_arr = [" + d.join(",\n") + "];\n";
        $("#encr").prop("checked") ? (g = Ba(L).map(
                function (b) {
                    return JSON.stringify(b)
                }), l = ['"X:1"']) : (g = [""], l = L.map(function (b) {
            return JSON.stringify(b)
        }));
        g = "abc_enc = [" + g.join(",\n") + "];\n";
        ...
        if ($("#drpuse").prop("checked"))
            ...
            try {
                b = document.createElement("a"), b.href = x, b.download = z + ".js", b.text = "Save synchronization data", $("#saveDiv").append(b), b.click()
            } catch (r) {
                confirm("Do you want to save your synchronization data?") && (document.open("text/html"), document.write("<pre>" + d + "</pre>"), document.close())
            }
    }
    ...
    function Da() {
        ...
        if (1 < b.length) {
            ...
            for (g = 0;
                    g < l.length;
                    g++)
                b = l[g].replace(/d:(\w{15}\/[^.]+\.)/, "https://dl.dropboxusercontent.com/s/$1"), (e = b.match(/xml=(.*)/)) ? c = decodeURIComponent(e[1]).replace("www.dropbox", "dl.dropboxusercontent") : (e = b.match(/med=(.*)/)) ? h = e[1] : (e = b.match(/tmr=(.*)/)) ? m.top_margin = parseInt(e[1]) : (e = b.match(/tb=([\d.]*)/)) ?
                        ...
                        (c = d, d = ""), /(\.mp3$)|(\.mp4$)|(\.ogg$)|(\.webm$)/.test(d) && (h = d, d = "");
            ...
            c ? $.get(c, "", null, "text").done(function (b, c) {
                ...
                y()
                    ...
                    function (b, c, e) {
                        $("#wait,#err").append("preload failed: " + e + ", trying script tag ...\n");
    ...
    function y() {
        ...
        "undefined" != typeof media_file && media_file && !opt.nomed && (F(media_file, media_file), opt.btns = 0);
    ...
    function Ea(b) {
        r ? P("websocket already open") : (r = new WebSocket("ws://" + b + ":8091/"), r.onmessage = function (b) {
            "master" == b.data ? $("#mbar").css("background", "rgba(255,0,0,0.2)") :
                    K(b.data, 100 * opt.delay)
        }, r.onerror = function (b) {
            P("socket error (server inaccessible?)");
            r = null
        }, r.onopen = function (b) {
            $("#mbar").css("background", "rgba(0,255,0,0.2)");
            opt.mstr && r.send("master");
            P("connection opened")
        }, r.onclose = function (b) {
            $("#mbar").css("background", "");
            P("connection closed: " + b.code);
            r = null
        })
    }
    function Fa(b) {
        if (!qa() && "meddiv" == b.target.id) {
            $("#buttons").css("opacity", "0.5");
            $("#streep").css("opacity", "1.0");
            var c = "touchstart" == b.type, d = $("#meddiv"), f = c ? b.originalEvent.touches[0].clientY : b.pageY, e = $("#buttons").height();
            d.css("cursor", "row-resize");
            d.on("mousemove touchmove", function (b) {
                var d = $("body").height();
                opt.media_height = (100 * (e + (c ? b.originalEvent.touches[0].clientY : b.pageY) - f) / d).toFixed() + "%";
    ...
    function A(b) {
        ...
        c = w.map(function (b, c) {
            return{x: Math.abs(b - opt.speed), i: c}
        }).sort(function (b, c) {
            return b.x - c.x
        })[0].i;
        -1 == b && 0 < c && (opt.speed = w[c + b]);
        1 == b && c < w.length - 1 && (opt.speed = w[c + b]);
        0 == b && (opt.speed = w[c]);
        $("#speed").val(opt.speed.toFixed(2));
        h && !p && (h.playbackRate = opt.speed);
        h && p && h.setPlaybackRate(opt.speed)
    }
    function ia() {
        k && k.drawTags();
        opt.lopctl = $("#lopctl").prop("checked");
        $("#atag").css("display", opt.lopctl ? "block" : "none");
        $("#btag").css("display", opt.lopctl ?
                "block" : "none")
    }
    function ya() {
        var b = Dropbox.createChooseButton({success: ua, cancel: function () {}, linkType: "direct", multiselect: !1, extensions: [".xml", ".abc", ".txt", ".js"]}), c = Dropbox.createChooseButton({success: function (b) {
                R("dbx", b)
            }, cancel: function () {}, linkType: "preview", multiselect: !1, extensions: [".ogg", ".mp3", ".webm", ".mp4"]});
        $("#abcfile").append(b);
        $("#mediafile").append(c)
    }
    function ra(b) {
    ...
    function qa() {
        ...
        b && ($("#help").toggleClass("showhlp", !1), setTimeout(sa, 0));
        return b
    }
    var u, M, k, n, Q, L, h, z, G, p = 0, w = [], N = 0, ea, m = {}, r = null, J, E = new C;
    opt = {};
    onYouTubeIframeAPIReady =
            function () {
                G = new YT.Player("vidyub", {events: {onReady: function () {
                            $("#yubuse").prop("checked", !0);
                            fa();
                            da()
                        }, onStateChange: function (b) {
                            b.data == YT.PlayerState.PLAYING ? (E.setKlok(H, 100), A(0)) : E.pause();
                            b.data == YT.PlayerState.CUED && t()
                        }}})
            };
    q.prototype.setline = function (b) {
        $("#wijzer").remove();
        this.sety(this.ymin[b], this.ymax[b]);
        this.setx(0, 0, 0);
        this.line = b;
        this.wijzer.prependTo(M[b]);
        this.width = u[b].width.baseVal.value;
        var c = $("#notation"), d = c.scrollTop(), f = d + c.height() - this.vmargin;
        (this.line_offsets[b + 1] > f || this.line_offsets[b] < d + this.vmargin) && c.scrollTop(this.line_offsets[b] - this.tmargin)
    };
    q.prototype.sety = function (b, c) {
        this.wijzer.attr("y", b.toFixed(2));
        this.wijzer.attr("width", "2");
        this.wijzer.attr("height", (30 + c - b).toFixed(2));
        this.shade.attr("fill", "blue")
    };
    q.prototype.setx = function (b, c, d) {
        var f = $("#notation"), e = f.scrollLeft(), h = e + f.width() - this.hmargin;

        // cursore metronome?
        opt.lncsr ? (this.wijzer.attr("x", b.toFixed(2)), this.wijzer.attr("width", "2"), this.shade.attr("fill-opacity", this.noCursor ? "0.0" : "0.5"), b /=
                this.scale, (b > h || b < e + this.hmargin) && f.scrollLeft(b - this.hmargin)) : (this.wijzer.attr("x", c.toFixed(2)), this.wijzer.attr("width", (d - c).toFixed(2)), this.shade.attr("fill-opacity", this.noCursor ? "0.0" : "" + opt.opacity), c /= this.scale, d /= this.scale, (d > h || c < e + this.hmargin) && f.scrollLeft(c - this.hmargin))
    };
    q.prototype.time2x = function (b, c) {
        if (!N) {
            this.cursorTime = b;
            var d, f, e, l, g;
            d = this.times;
            for (g = this.time_ix; g < d.length && b > d[g]; )
                g += 1;
            if (g == d.length)
                p ? 1 == h.getPlayerState() && h.pauseVideo() : h.paused || h.pause();
            else {
                for (; 0 < g && b < d[g - 1]; )
                    --g;
                c && .3 > d[g] - b && (d[g] = b - .01, console.log("tijdcor: " + (b - .01) + ", maat: " + g), g < d.length - 1 && (g += 1));
                opt.metro && g != this.time_ix && za(g, b);
                this.time_ix = g;
                this.repcnt = this.tixlb[g][2];
                this.msre = l = this.tixlb[g][1];
                f = this.tixlb[g][0];
                this.line != f && this.setline(f);
                e = this.xs[f];
                f = d[g - 1];
                g = d[g];
                d = e[l - 1] + 10;
                ...
                0 >= b || b > e ? this.setx(0, 0, 0) : this.setx(f, d, l);
                opt.synbox && this.showSyncInfo()
            }
        }
    };
    q.prototype.drawTags = function () {
        for (var b in{atag:1, btag:1})
            b in lpRec && (a = lpRec[b], this[b].prependTo(M[a.line]), this[b].attr("x", a.x), this[b].attr("y", this.ymin[a.line]))
    };
    q.prototype.doLoopTag = function (b, c, d, f, e) {
        function h(b, c, d, e, f, g, l, k) {
            if (!opt.lncsr) {
                var n = m.xs[d];
                g = m.times;
                c = n[l - 1];
                ...
                l = g[k - 1];
                k = g[k];
                lpRec.loopStart == l + .01 && (b = "btag", f = "loopEnd");
                lpRec.loopEnd == k - .01 && (b = "atag", f = "loopStart");
                "loopStart" == f ? g = l + .01 : (c = n, g = k - .01)
            }
            lpRec[b] = {x: c.toFixed(2), line: d};
            lpRec.loopBtn = e;
            lpRec[f] = g;
            m.drawTags()
        }
        ...
        switch (lpRec.loopBtn) {
            case 1:
                h("atag", b, c, 2, "loopStart", d, f, e);
                break;
            case 2:
                d > lpRec.loopStart && h("btag", b, c, 3, "loopEnd", d, f, e);
                break;
            case 3:
                g = Math.abs(lpRec.loopStart - d), k = Math.abs(lpRec.loopEnd - d), g < k ? h("atag", b, c, 3, "loopStart", d, f, e) : h("btag", b, c, 3, "loopEnd", d, f, e)
            }
    };
    q.prototype.x2time = function (b, c) {
        var d, f, e, l, g, k;
        b *= this.scale;
        d = this.xs[c];
        e = 1;
        if (b < d[0])
            ...
            for (; e < d.length && d[e] < b; )
                e += 1;
            e == d.length ? O({keyCode: 80}) : (k = this.lbtix[c][e], k[this.repcnt] || (this.repcnt = 1), k = k[this.repcnt], f = this.times, l = d[e - 1], g = d[e], d =
            f[k - 1], f = f[k], l = d + (f - d) * (b - l) / (g - l), opt.lopctl ? this.doLoopTag(b, c, l, e, k) : opt.synbox && (p ? h.getPlayerState() == YT.PlayerState.PLAYING : !h.paused) ? this.syncTimes(b, e, c, k) : U(!1, (opt.lncsr ? l : d + .01) + n))
        }
    };
    q.prototype.goMsre = function (b) {
        var c = this.time_ix;
        h && (b = b ? this.times[c] + .01 : 2 >= c ? .01 : this.times[c - 2] + .01, U(!1, b + n))
    };

    q.prototype.showSyncInfo = function () {
        var b = this.time_ix, b = this.times[b] - this.times[b - 1];
        $("#sync_info").html("duration&nbsp;measure:<br>" + b.toFixed(3) + " sec.<br>");
        $("#sync_info").append("media&nbsp;offset:<br>" + n.toFixed(3) + " sec.")
    };
    q.prototype.changeTimesKeyb = function (b) {
        this.changeTimes(this.lbtix[this.line][this.msre][this.repcnt] - 1, b, 0)
    };
    q.prototype.changeTimes = function (b, c, d) {
        var f, e = this.times;
        for (b += 1; b < e.length; ++b)
            f = d ? e[b - 1] + d : e[b] + c, e[b] = f
    };
    q.prototype.syncTimes = function (b, c, d, f) {
        b = this.lbtix[d][c][this.repcnt] - 1;
        d = (p ? h.getCurrentTime() : h.currentTime) - n - .2;
        0 == b ? (n += d, p ? h.seekTo(n + .01, !0) : h.currentTime = n + .01, N && $("#woff").click()) : (--b, c = 2 == f ? 0 : this.times[f - 2], f = this.times[f - 1], d < c + .5 ? alert("tempo faster than 240 bpm: first sync previous measures") :
                (this.lastSync > b ? this.changeTimes(b, d - f, 0) : (this.changeTimes(b, 0, d - c), this.lastSync = b), opt.jump && (p ? h.seekTo(c + n + .01, !0) : h.currentTime = c + n + .01)))
    };
    q.prototype.setSize = function () {
        var b, c, d, f, e;
        for (b = 0; b < u.length; ++b)
            c = u[b], d = c.width.baseVal.value, f = c.height.baseVal.value, e = $("#notation").prop("clientWidth"), c.width.baseVal.value = e, c.height.baseVal.value = e * f / d
    };
    q.prototype.setScale = function () {
        var b, c, d, f;
        c = u[0];
        ...
        this.scale = c / d / b;
        d = $("#notation").position();
        f = $("#notation").scrollTop();
        this.line_offsets = [];
        for (b = 0; b < u.length; ++b)
            c = $(u[b]).position(), this.line_offsets[b] = f + c.top - d.top;
        this.line_offsets[b] = $("#notation").prop("scrollHeight")
    };
    q.prototype.compCountIn = function () {
        var b = {time: .25, num: 4}, c = 1 < this.time_ix ? this.time_ix - 1 : this.time_ix, d = Math.min(this.times.length - 1, c + 3);
        if (d > c) {
            var f = this.tixbts.slice(c, d).reduce(function (b, c) {
                return b + c
            }, 0);
            b.time = (this.times[d] - this.times[c]) / f / opt.speed;
            b.num = this.tixbts[c]
        }
        return b
    };
    ...
    C.prototype.setKlok = function (b, c) {
        -1 != this.klok && clearInterval(this.klok);
        this.klok = b ? setInterval(b, c) : -1;
        this.paused = !1;
        k && opt.nocsr && (k.noCursor = 1)
    };
    C.prototype.clearKlok = function () {
        -1 != this.klok && clearInterval(this.klok);
        this.klok = -1;
        this.paused = !0;
        k && (k.noCursor = 0)
    ...
    $(document).ready(function () {
        ...
        $("#lopctl").click(ia);
        var b = '<a href="http://wim.vree.org/js/">abcweb</a> (version: ' + msc_VERSION + ")</br>\u00a9Willem Vree", b = b + '<br>using:<br><a href="http://moinejf.free.fr/js/">abc2svg</a>, \u00a9Jef Moine';
        ...
        $("#menu label").toggle();
        $("#mbar").click(function () {
            "none" == $("#menu label").css("display") ? $("#menu label").toggle(!0) : sa()
        });
